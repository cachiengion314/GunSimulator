...
public partial class GridWorld : MonoBehaviour
{
  ...
  /// as obstacles. Note that for the performance wise excludes own item is int2 
  ...
  NativeList<int2> ConvertFromNodeToPosList(NativeList<GridNode> visitedNodes)
  {
    var maxLength = 32;
    NativeList<int2> visitedGridPos = new(maxLength, Allocator.Temp);
    for (int i = 0; i < visitedNodes.Length; ++i)
    {
      visitedGridPos.Add(visitedNodes[i].GridPosition);
    }
    visitedNodes.Dispose();
    return visitedGridPos;
  }
  ...
  NativeList<GridNode> FindNeighborNodesFor(in GridNode node, int2[] excludes)
  {
    ...
    for (int i = 0; i < directions.Length; ++i)
    {
      ...
      // if (ItemSystem.Instance.TableCells[_index] == null) continue;
      // if (ItemSystem.Instance.Cups[_index] != null) continue;
      // if (ItemSystem.Instance.Bottles[_index] != null) continue;
      // if (ItemSystem.Instance.Boxes[_index] != null) continue;
